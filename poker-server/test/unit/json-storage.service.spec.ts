...
describe('JsonStorageService', () => {
  ...
  const seedSeatedButNotDealtLeakScenario = async (roomId: string) => {
    await seedCompletedHands(roomId);

    const handTwoPath = path.join(testRoomsDir, roomId, 'hands', '2.jsonl');
    const handTwoEvents = [
      {
        recordId: 'seated-not-dealt-hand-2-started',
        seq: 1,
        roomId,
        handNumber: 2,
        street: 'PRE_FLOP',
      ...
      {
        recordId: 'seated-not-dealt-hand-2-action',
        seq: 2,
        roomId,
        handNumber: 2,
        street: 'PRE_FLOP',
        ...
        payload: {
          ...
          request: {
            action: 'fold',
            actionId: 'seated-not-dealt-hand-2-action-1',
          },
          decision: {
            currentPlayerTurnBefore: 'bob',
            playerStatusBefore: 'connected',
      ...
      {
        recordId: 'seated-not-dealt-hand-2-settled',
        seq: 3,
        roomId,
        handNumber: 2,
        street: 'PRE_FLOP',
  ...
  describe('saved game archives', () => {
    ...
    it('excludes seated-but-not-dealt hands from saved-game ownership and review targets', async () => {
      const roomId = 'ROOMARCHIVE-SEATED-NOT-DEALT';
      await seedSeatedButNotDealtLeakScenario(roomId);
    ...
    it('suppresses seated-but-not-dealt leaked hands from legacy archive reads without dealtPlayerIds', async () => {
      const roomId = 'ROOMARCHIVE-SEATED-NOT-DEALT-LEGACY';
      await seedSeatedButNotDealtLeakScenario(roomId);
      ...
      if (!leakedHand) {
        throw new Error('Expected seated-but-not-dealt leaked hand source');
      }

      delete leakedHand.history.dealtPlayerIds;
      archiveRecord.handCount = 2;
      archiveRecord.playerViews['user-alice'].hands.push(leakedHand);
      await fs.writeFile(archivePath, JSON.stringify(archiveRecord, null, 2));
