...
const createRoom = (params: {
  ...
  currentHand: null,
  readyPhase: 'START_GAME' as const,
...
describe('EventsGateway randomize seats', () => {
  ...
  it('rejects randomize seats when game is already in progress', async () => {
    const room = createRoom({
      hostId: 'p-host',
    ...
    expect(result).toEqual({
      success: false,
      error: 'Seat order can only be changed before game starts',
    });
    expect(gameService.shuffleSeatOrder).not.toHaveBeenCalled();
    expect(hostClient.emit).toHaveBeenCalledWith('ERROR', {
      message: 'Seat order can only be changed before game starts',
    });
  });
});
